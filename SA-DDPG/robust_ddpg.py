...
class RobustDeterministicActorCriticNet(nn.Module, BaseNet):
    def __init__(self,
        ...
        if self.use_loss_fusion:
            ...
            use_rp = rp_cfg is not None and rp_cfg.get('enabled', False)
            if use_rp:
                rp_dim_factor=rp_cfg.get('dim_factor', 8)
                rp_out = rp_cfg.get('out_dim', 2 ** math.ceil(math.log2(state_dim * rp_dim_factor)))

                ln_eps   = rp_cfg.get('ln_eps', 1e-5)
                ln_aff   = rp_cfg.get('ln_affine', True)

                rp_lambda = rp_cfg.get('lambda', 1e-4)
                print(f"Using Random Projection with input state with dim = {state_dim}, rp_out_dim={rp_out}, rp_lambda={rp_lambda}")
                actor_core = nn.Sequential(
                    RandomProjection(state_dim, rp_out, rp_lambda, bias=False),
                    nn.LayerNorm(rp_out, eps=ln_eps, elementwise_affine=ln_aff),
                    *generate_mlp_units(rp_out, actor_network, action_dim)
                )
            else:
                actor_core = model_mlp_any(state_dim, actor_network, action_dim)

            self.fc_action = BoundedModule(actor_core, (torch.empty(size=(1, state_dim)),), device=Config.DEVICE)
