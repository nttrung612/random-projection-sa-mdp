...
class RandomProjection(nn.Linear):
    def __init__(self, in_features, out_features, scale, bias=False):
...
class QNetwork(nn.Module):
    def __init__(self, name, env, input_shape, num_actions, robust=False, width=1, proj_dim=8192, proj_lambda=1.0):
        ...
        if name == 'DQN':
            ...
            self.features = nn.Sequential(
                ...
                RandomProjection(512*width, proj_dim*width, scale=proj_lambda),
                nn.LayerNorm(proj_dim*width),
                nn.ReLU(),
                nn.Linear(proj_dim*width, self.num_actions)
            )
        elif name == 'DuelingCnnDQN':
            self.features = DuelingCnnDQN(input_shape, num_actions, width, proj_dim, proj_lambda)
        else:
...
class DuelingCnnDQN(nn.Module):
    def __init__(self, input_shape, num_actions, width=1, proj_dim=8192, proj_lambda=1.0):
        ...
        self.cnn = nn.Sequential(
            ...
            nn.ReLU(),
            Flatten(),
            # RandomProjection(3136*width, proj_dim*width, scale=proj_lambda),
            # nn.ReLU()
        )
        ...
        self.advantage = nn.Sequential(
            # nn.Linear(proj_dim*width, 512*width),
            nn.Linear(3136*width, 512*width),
            # RandomProjection(512*width, proj_dim*width, scale=proj_lambda),
            # nn.LayerNorm(proj_dim*width),
            nn.ReLU(),
...
def model_setup(env_id, env, robust_model, logger, use_cuda, dueling=False, model_width=1, proj_dim=8192, proj_lambda=1.0):
    ...
    model = QNetwork(net_name, env, env.observation_space.shape, env.action_space.n, robust_model, model_width, proj_dim, proj_lambda)
    if use_cuda:
        model = model.cuda()
    print(model)
