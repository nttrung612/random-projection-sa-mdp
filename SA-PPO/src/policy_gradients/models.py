...
class CtsPolicy(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, init, hidden_sizes=HIDDEN_SIZES,
                 ...
                 rp_dim_factor=2, rp_lambda=1e-4, use_up_projection=False):
        super().__init__()
        if isinstance(activation, str):
            self.activation = activation_with_name(activation)()
        else:
        ...
        if use_up_projection:
            assert rp_dim_factor is not None
            rp_dim = 2 ** math.ceil(math.log2(state_dim * rp_dim_factor))
            print(f"Using UpProjection: in={state_dim}, out={rp_dim}, lamda={rp_lambda}")
            self.rp = nn.Linear(state_dim, rp_dim)
            # nn.init.orthogonal_(self.rp.weight, gain=rp_lambda)
            self.rp_ln = nn.LayerNorm(rp_dim)
            prev_size = rp_dim
        elif rp_dim_factor is not None: 
            rp_dim = 2 ** math.ceil(math.log2(state_dim * rp_dim_factor)) 
            print(f"Using RandomProjection: in={state_dim}, out={rp_dim}, lamda={rp_lambda}") 
            self.rp = RandomProjection(state_dim, rp_dim, scale=rp_lambda) 
            self.rp_ln = nn.LayerNorm(rp_dim)
            prev_size = rp_dim 
        else: 
            self.rp = None 
